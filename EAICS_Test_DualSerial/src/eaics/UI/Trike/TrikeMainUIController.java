...
public class TrikeMainUIController extends MainUIController
{
        ...
        try 
	{
            ...
            if(serial0.getType()==SERType.THROTTLE) {
                settingsPageController.initData(loadCell, serial0, logging);
            }
            else {
                 settingsPageController.initData(loadCell, serial1, logging);
            }
    ...
    private void handleTarePressed(ActionEvent event) {
        ...
        else if(serial1.getType()==SERType.THROTTLE) {
            serial1.writeData("0");
        }
    }
    ...
    public void initData(Logging logging, Serial serial0, Serial serial1) throws IOException 
    {
        this.logging = logging;
        this.serial0 = serial0;
        this.serial1 = serial1;
        if(serial0.getType()==SERType.THROTTLE) {
            this.loadCell = serial0.getCell();
            this.throttle = serial0.getThrottle();
        }
        else {
            this.loadCell = serial1.getCell();
            this.throttle = serial1.getThrottle();
        }
        if(this.throttle.isSendingThrottleCommands())
        {
            startStopThrottle.setText("Disable Throttle");
