  ...
  void temp() {
    ...
    float average[6];
     ...
     for(int j=0; j<6; j++) {
        ...
        steinhart[j] = average[j] / THERMISTORNOMINAL;     // (R/Ro)
        steinhart[j] = log(steinhart[j]);                  // ln(R/Ro)
        steinhart[j] /= BCOEFFICIENT;                   // 1/B * ln(R/Ro)
        steinhart[j] += 1.0 / (TEMPERATURENOMINAL + 273.15); // + (1/To)
        steinhart[j] = 1.0 / steinhart[j];                 // Invert
        steinhart[j] -= 273.15;                         // convert to C
    }
  }
